...
export class Graph extends React.Component {
    ...
    createGraph(graph: Lnd.Graph) {
        ...
        // during rendering
        this.nodes = [];
        const lightningNodes = graph["nodes"];
        console.log("<===========", lightningNodes)
        lightningNodes.map((lightningNode, index) => {
            const x: D3Node = {
                id: lightningNode.pub_key,
                color: lightningNode.color,
                title: lightningNode.alias
            };
            this.nodes.push(x);
        });
        ...
        // during rendering
        this.links = [];
        const lightningChannels = graph["edges"];
        console.log("====> ")
        console.log("====> ", lightningChannels)
        lightningChannels.map((lightningChannel, index) => {
            const y : D3Link = {
                id: lightningChannel.channel_id,
                source: lightningChannel.node1_pub,
                target: lightningChannel.node2_pub
            };
            this.links.push(y);
        });
